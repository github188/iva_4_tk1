...
{
	flv_buffer *c = (flv_buffer *)malloc( sizeof(flv_buffer) );
	...
	c->data = (uint8_t *)malloc( FLV_BUFFER_DEF_SIZE );
	if( !c->data )
	{
		free( c );
		return NULL;
	}
	c->d_max = FLV_BUFFER_DEF_SIZE;
	c->d_cur = 0;
	c->d_total = 0;
...
int flv_append_data( flv_buffer *c, uint8_t *data, unsigned size )
{
	...
	if( ns > c->d_max )
	{
		...
		{
			dn <<= 1;
			//printf("ns:%u, d_max:%u --> dn:%u\n",ns, c->d_max, dn);
		}
		//printf("after while\n");
		printf("realloc ns:%u, d_max:%u --> dn:%u\n",ns, c->d_max, dn);
