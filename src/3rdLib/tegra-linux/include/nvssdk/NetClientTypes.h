...
#define	MAX_CHANNEL_NUM_TDWY		(MAX_LOCAL_CHANNEL_NUM + MAX_DIGIT_CHANNEL_NUM)	   //支持3511
...
//#define MAX_DATA_CHAN MAX_CHANNEL_NUM_TDWY		//包含副码流
...
#define VCA_CMD_SCENEREV						(VCA_CMD_MIN + 25)					//智能分析场景恢复时间
#define VCA_CMD_MAX								(VCA_CMD_MIN + 26)                  //最多
...
struct vca_TVCAParam
{
	...
	vca_TVCAChannelParam	chnParam[MAX_CHANNEL_NUM_TDWY];				//	每个通道的参数设置
};					
...
struct TDVR3GInfo
{
	...
	int		iGpsOverlay[MAX_CHANNEL_NUM_TDWY];			//	使能叠加GPS信息，与通道相关
...
typedef struct __tagAlarmLink
{
	...
	int				iPtzSize;										//PTZ
	struct			vca_TLinkPTZ		ptz[MAX_CHANNEL_NUM_TDWY];		//iLinkType=4时存储联动PTZ的参数
...
struct TITSRoadwayCapture			//抓拍位置
{
	...
	//5—识别第三张                        
	int iSceneID;							// 场景ID(0~15) 20140203扩展
	int iRedLightCapType;					//0-按方向抓拍（默认）1-按车道抓拍 2-按方向+车道
...
struct ITS_EnalbeParam	
{
	int iType;	    // 功能类型 0--电源同步功能  1--启动手动模式  2--CRC校验功能  3--视频测速功能   4—红灯增益使能   5—ftp使能
					// 6--图像合成功能  7--人脸识别功能  8--车标识别使能 9--安全带识别使能 10--副驾驶人脸识别功能 11--副驾驶人脸识别功能
					// 12--图片输出开关 13--车牌图片输出使能 14--主驾驶人脸图片输出使能 15--副驾驶人脸图片输出使能
...
#define CI_COMMON_ID_VCACARCHECK				0x12016 //过车检测使能
...
typedef struct
{
	...
	int				m_iVodTransFrameRate;   //帧率
	int				m_iVodTransStreamRate;  //码率
	int				m_iFileFlag;			//0:下载成多个文件 1:下载成单个文件
}DOWNLOAD_TIMESPAN;
...
typedef struct
{
	int				m_iBufSize;				//结构体大小
	int				m_iFileIndex;			//文件索引序号
	char			m_cFilenNme[255];		//本地录像文件名
	long long		m_lFileSize;			//文件大小,单位：字节
	NVS_FILE_TIME	m_tTimeBeg;				//开始时间
	NVS_FILE_TIME	m_tTimeEnd;				//结束时间
}DOWNLOAD_FILEINFO;
...
#define AUTOTEST_ZIGBEE						(CMD_AUTOTEST_MIN+45)	  //Zigbee外设检测；
#define AUTOTEST_TEMPERATURE				(CMD_AUTOTEST_MIN+46)	  //温度；
...
#define COLOR_AUTO_OUT		3			//自动外同步
#define COLOR_AUTO_ALARM	4			//报警同步
#define COLOR2GRAY_TIMED	5			//定时
