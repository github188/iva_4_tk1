...
#ifdef USE_ISAPI
/* ISAPI Response */
void send_r_isapi_json(request * req, char * resp_json)
{
	int body_len = 0;
	if(resp_json)
	{
		body_len = strlen(resp_json);
	}
	...
	req_write(req, "Content-Type: application/json; charset=\"UTF-8\" \r\n");
