...
void process_requests(int server_s)
{
    ...
    while (current) {
        time(&current_time);
#if 0		
		fprintf(stderr,"\n---buffer_start:%d buffer_end:%d status:%d client_stream:%s request_uri:%s method:%d content_type:%s content_length:%s path_info:%s\n",
			current->buffer_start,current->buffer_end,current->status,current->client_stream,current->request_uri,current->method,current->content_type,current->content_length,current->path_info);
#endif		
